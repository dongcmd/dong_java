...
class Buyer {
	...
	Food[] cart = new Food[1];
	
	void buy(Food food) {
		if(money < food.price) { System.out.println(food + " 구매시 잔액부족"); return; }
		money -= food.price;
		point += food.point;
		System.out.println(food.toString() + "를(을) " + food.price + "가격에 구입");
		if(food instanceof Fruit) { System.out.println( ( (Fruit)food ).brix +"당도 상품 구매"); }
		if(food instanceof Drink) { System.out.println( ( (Drink)food ).ml +"ml 상품 구매"); }
		if(food instanceof Snack) { System.out.println( ( (Snack)food ).gram +"g 상품 구매"); }
		cart = Arrays.copyOf(cart, cart.length+1);
		cart[cnt++] = food;
	}
	
	void summary() {
		int priceSum = 0, fSum = 0, dSum = 0, sSum = 0;
		...
		for(int i = 0; i < cart.length-1; i++) {
			...
			foodlist += cart[i].toString() + ",";
			if(cart[i] instanceof Fruit) { fSum += cart[i].price; fl += cart[i].toString() + ","; }
			if(cart[i] instanceof Drink) { dSum += cart[i].price; dl += cart[i].toString() + ","; }
			if(cart[i] instanceof Snack) { sSum += cart[i].price; sl += cart[i].toString() + ","; }
		}
		
		
		System.out.println("총 구매 금액 : " + priceSum);
		System.out.println("총 구매 목록 : " + foodlist);
		System.out.printf("과일 구매 금액: %d, 과일 구매 목록 : %s\n", fSum, fl);
		System.out.printf("음료 구매 금액: %d, 음료 구매 목록 : %s\n", dSum, dl);
		System.out.printf("과자 구매 금액: %d, 과자 구매 목록 : %s\n", sSum, sl);
